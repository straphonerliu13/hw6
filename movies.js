...
window.addEventListener('DOMContentLoaded', async function(event) {
    ...
    let url = `https://api.themoviedb.org/3/movie/now_playing?api_key=95f530e35223d79fbe74b4289c1ecbc5&language=en-US`
    ...
    for (let i=0; i<movies.length; i++) {
        ...
        // let docRef = await db.collection('movies').add({
        //    title: films
        // })
    ...
    // ⬇️ ⬇️ ⬇️
        ...
        document.querySelector(`.mov-${movieID} .watched-button`).addEventListener('click', async function(event) {
            ...
            console.log(`Movie with ID ${movieID} has been watched!`)
            document.querySelector(`.mov-${movieID}`).classList.add('opacity-0')

            if (document.querySelector(`.mov-${movieID}`).classList.contains('opacity-0') == true) {
                document.querySelector(`.mov-${movieID} .watched-button`).addEventListener('click', function(event) {
                    event.preventDefault()
                    document.querySelector(`.mov-${movieID}`).classList.contains('opacity-0')
                    console.log(`Movie with ID ${movieID} has been un-watched!`)
                    document.querySelector(`.mov-${movieID}`).classList.remove('opacity-0')
                })
            }
        ...
        let querySnapshot = await db.collection('movies').doc(`${movieID}`).set({
        })
